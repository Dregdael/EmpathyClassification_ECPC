...
def evaluate(model, data_loader, device):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            #print(y['empathy']+1)
    return accuracy_score(actual_labels, predictions), classification_report(actual_labels, predictions),cem.get_cem(predictions,y)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        accuracy, report, cem = evaluate(model, val_dataloader, device)
    ...
    accuracy, report, cem = evaluate(model, test_dataloader, device)
